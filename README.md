...
- **주요 발견 사항 (Deadlock)**: 
    ...
    - **개선 방안**: 
        1. **ID-based Sequential Ordering**: 사용자 ID가 낮은 순서대로 고정된 락 획득 순서를 보장하여 모든 거래 상황에서 **교착 상태(Deadlock)를 100% 방지**합니다.
        2. **Redis Distributed Lock (Future)**: 상용 수준의 엔진에서는 DB 락 대신 Redisson 등을 활용한 분산 락으로 애플리케이션 계층에서 순차성을 보장하는 것이 효율적입니다.
...
- **해결 과정**:
    - **고정 잠금 순서(ID-based Sequential Locking)**: 단순히 ID가 낮은 순서로 락을 거는 규칙을 도입했습니다. 초기에는 특정 유저가 항상 우선권을 갖는 불공정성(Starvation)을 우려해 XOR 동적 방식을 검토했으나, 데이터베이스의 락 큐가 **FIFO(First-In-First-Out)**로 작동함을 활용하여 **'공정성'과 '시스템 안전성(Deadlock-free)'을 동시에 확보**했습니다.
- **교훈**: 동시성 제어 시스템에서는 '동적인 공정성'보다 **'전역적인 일관성(Global Consistency)'**이 데드락 방지의 가장 확실한 기초임을 확인했습니다.
